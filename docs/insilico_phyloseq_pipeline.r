...
## Verify that your accompanying environmental file has AT LEAST the following:
...
# for this pipeline we assume that you have flagged you blanks and sample with
# these exact names
# - one or more factors meaningful for your experimental design (something
...
## Merging of multiple runs (if necessary). Import and merge the different phyloseq
# objects into a single object. Import and merge as many objects as needed
# Using read_csv of readr package to import the large csv file as read.csv is very slow

otu1 <- read_csv("seqtab1.csv", col_names = TRUE)
tax1 <- read_csv("taxa1.csv", col_names = TRUE)

otu2 <- read_csv("seqtab2.csv", col_names = TRUE)
tax2 <- read_csv("taxa2.csv", col_names = TRUE)

# Add row names and remove first column to comply with phyloseq format
...
seqtab1 <- phyloseq(otu_table(as.matrix(otu1), taxa_are_rows = F),
                     tax_table(as.matrix(taxa1)))
seqtab2 <- phyloseq(otu_table(as.matrix(otu2), taxa_are_rows = F),
                     tax_table(as.matrix(taxa2)))
seqtab1
seqtab2

# Merging
prok_data_raw <- merge_phyloseq(seqtab1, seqtab2) # Merging of different phyloseq objects
prok_data_raw

# Verify that your envdataset file has the same row names as the phyloseq
# object station names
colnames(otu_table(prok_data_raw))

# import your environmental data file
env_dataset <- read.csv("dataset_env.csv", header=T, sep=",", row.names=1)

 # check the imported file to see if factors have been impoted correctly
summary(env_dataset)

# Add to the phyloseq object the environmental data
prok_data_raw <- phyloseq(prok_data_raw, sample_data(env_dataset))
...
plot_bar(subset_samples(prok_data_raw, sample_names(prok_data_raw) == "blank"), fill="Phylum", title = "Diversity in the blank") +
theme_hc() +
...
blanks = subset_samples(prok_data_raw, sample_names(prok_data_raw) == "blank")
samples = subset_samples(prok_data_raw, sample_names(prok_data_raw) != "blank")
...
plot_bar(bac_ra_phyla, fill="Phylum", title = "Diversity at Phylum level") +
theme_hc()
...
plot_richness(prok_ndata, x="factor1", measures =c("Simpson"), color="area") +
   geom_boxplot() +
   geom_jitter() +
   theme_hc()
...
plot_bar(phyla_top10, fill="Family", x="station", title = "Diversity of the top 10 phyla") +
    theme_hc()
...
plot_bar(subset_taxa(prok_ra_phyla, Phylum == "Proteobacteria"), fill="Order", x="station", title = "Order level diversity within the Proteobacteria") +
    theme_hc() + theme(legend.position = "none")
as_ggplot(get_legend(plot_bar(subset_taxa(prok_ra_phyla, Phylum == "Proteobacteria"), fill="Order", x="station", title = "Diversity at Class level")))
...
plot(scores(prok.v_nmds_w)[,1], data.frame(sample_data(prok_ndata))$temp, main="Plot of the nMDS ordination scores against Temperature")
abline(lsfit(scores(prok.v_nmds_w)[,1], data.frame(sample_data(prok_data))$temp)
cor.test(scores(prok.v_nmds_w)[,1], data.frame(sample_data(prok_data))$temp)

# Temperature and dimension 2
plot(scores(prok.v_nmds_w)[,2], data.frame(sample_data(prok_ndata))$temp, main="Plot of the nMDS ordination scores against Temperature")
abline(lsfit(scores(prok.v_nmds_w)[,2], data.frame(sample_data(prok_data))$temp)
cor.test(scores(prok.v_nmds_w)[,2], data.frame(sample_data(prok_data))$temp)
...
with(data.frame(sample_data(prok_ndata)), ordisurf(prok.v_nmds_w, temp, add = TRUE, col = "red"))
...
plot(igraph::simplify(prok.cor_p6.ig), layout=layout_with_fr, vertex.color=my_color_g, vertex.size = 4, vertex.label = NA)
...
env_data_spearman <- cor(as.data.frame(sample_data(prok_ndata))[,c(4:5)], method = "spearman", use = "complete.obs")
